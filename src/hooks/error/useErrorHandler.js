...
export const useErrorHandler = () => {
  ...
  const retry = useCallback(
    async (operation, maxRetries = 3, context = {}) => {
      let attempt = 0;
  ...
  return {
    ...
    highPriorityErrors: errors.filter(
      (err) =>
        err.severity === ERROR_SEVERITY.CRITICAL ||
        err.severity === ERROR_SEVERITY.HIGH
    ),
  };
};
