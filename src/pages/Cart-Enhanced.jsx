...
import { updateQuantity, removeFromCart, clearCart, applyCoupon, removeCoupon } from '../store/slices/cartSlice';
import { Button, Card, CardBody, Badge, Alert, Modal, Breadcrumb, ProgressBar } from '../components/ui';
...
const CartEnhanced = () => {
  ...
  const { items: cartItems, coupon, isLoading } = useSelector((state) => state.cart || { items: [], coupon: null, isLoading: false });
  ...
  // Mock delivery slots
  const deliverySlots = [
    { id: 'today-morning', label: 'Today 8AM - 12PM', price: 50, available: true },
    { id: 'today-evening', label: 'Today 4PM - 8PM', price: 50, available: false },
    { id: 'tomorrow-morning', label: 'Tomorrow 8AM - 12PM', price: 30, available: true },
    { id: 'tomorrow-evening', label: 'Tomorrow 4PM - 8PM', price: 30, available: true },
    { id: 'day-after-morning', label: 'Day After Tomorrow 8AM - 12PM', price: 20, available: true },
    { id: 'standard', label: 'Standard Delivery (2-3 days)', price: 0, available: true }
  ];

  // Mock available coupons
  const availableCoupons = [
    { code: 'FRESH20', discount: 20, type: 'percentage', minOrder: 300, description: '20% off on orders above ₹300' },
    { code: 'FIRST50', discount: 50, type: 'fixed', minOrder: 200, description: '₹50 off on your first order' },
    { code: 'ORGANIC15', discount: 15, type: 'percentage', minOrder: 500, description: '15% off on organic products' }
  ];
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (subtotal < validCoupon.minOrder) {
        setCouponError(`Minimum order amount ₹${validCoupon.minOrder} required`);
        return;
      }
  ...
  const selectedSlot = deliverySlots.find(slot => slot.id === selectedDeliverySlot);
  const deliveryCharge = selectedSlot ? selectedSlot.price : (subtotal >= 500 ? 0 : 50);
      ...
      <div className="min-h-screen" style={{ backgroundColor: 'var(--neutral-50)' }}>
        <div className="container py-12">
          <div className="max-w-2xl mx-auto text-center">
            <div className="w-32 h-32 mx-auto mb-8 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--neutral-100)' }}>
              <svg className="w-16 h-16" style={{ color: 'var(--neutral-400)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m1.6 8L5 3H3m4 10v6a1 1 0 001 1h7a1 1 0 001-1v-6m-8 0V9a1 1 0 011-1h6a1 1 0 011 1v4.01" />
              </svg>
            </div>
            
            <h1 className="text-3xl font-bold mb-4" style={{ color: 'var(--neutral-900)' }}>
              Your cart is empty
            </h1>
            
            <p className="text-lg mb-8" style={{ color: 'var(--neutral-600)' }}>
              Discover fresh, organic produce from local farmers and start building your healthy lifestyle!
            </p>
        ...
        <div className="mb-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold" style={{ color: 'var(--neutral-900)' }}>
              {cartStep === 'cart' ? 'Shopping Cart' : 
               cartStep === 'delivery' ? 'Select Delivery' : 'Order Summary'}
            </h1>
          ...
          <div className="lg:col-span-2">
            ...
            {cartStep === 'cart' && (
              <>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                <div className="flex items-center border rounded-lg" style={{ borderColor: 'var(--neutral-300)' }}>
                                  <button
                            ...
                            {item.stock && (
                              <div className="mt-2">
                                <Badge 
                                  variant={item.stock > 10 ? 'success' : item.stock > 0 ? 'warning' : 'error'}
                                  className="text-xs"
                                >
                ...
                <Card className="mb-6">
                  <div className="p-6">
                    <h3 className="font-semibold mb-4" style={{ color: 'var(--neutral-900)' }}>
                      Apply Coupon
                    </h3>
                    
                    {coupon ? (
                      <div className="flex items-center justify-between p-4 rounded-lg" style={{ backgroundColor: 'var(--success-50)' }}>
                        <div className="flex items-center gap-2">
                          <svg className="w-5 h-5" style={{ color: 'var(--success-600)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <div>
                            <div className="font-medium" style={{ color: 'var(--success-700)' }}>
                              {coupon.code} Applied
                            </div>
                      ...
                      <div className="space-y-4">
                        ...
                        {/* Available Coupons */}
                        <div>
                          <h4 className="text-sm font-medium mb-2" style={{ color: 'var(--neutral-700)' }}>
                            Available Coupons:
                          </h4>
                          <div className="space-y-2">
                            {availableCoupons.map((couponItem) => (
                              ...
                              >
                                <div className="flex justify-between items-center">
                                  <div>
                                    <div className="font-medium text-sm" style={{ color: 'var(--primary-600)' }}>
                                      {couponItem.code}
                                    </div>
                                  ...
                                  <Badge variant="info" className="text-xs">
                                    {couponItem.type === 'percentage' ? `${couponItem.discount}% OFF` : `₹${couponItem.discount} OFF`}
                                  </Badge>
                                </div>
                              </div>
                            ))}
            ...
            {cartStep === 'delivery' && (
              <Card>
                <div className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <input
                              ...
                              onChange={() => slot.available && setSelectedDeliverySlot(slot.id)}
                  ...
                  <div className="mt-6 p-4 rounded-lg" style={{ backgroundColor: 'var(--info-50)' }}>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm" style={{ color: 'var(--info-700)' }}>
                        ...
                        <ul className="space-y-1 text-xs">
                          ...
                          <li>• Same-day delivery available for limited areas</li>
                          <li>• You'll receive SMS updates for your delivery</li>
                        </ul>
                      </div>
                    </div>
                  </div>
            ...
            {cartStep === 'summary' && (
              <Card>
                <div className="p-6">
                  ...
                  {/* Delivery Info */}
                  {selectedSlot && (
                    <div className="p-4 border rounded-lg mb-6" style={{ borderColor: 'var(--neutral-200)', backgroundColor: 'var(--neutral-50)' }}>
                      ...
                      <div className="text-sm" style={{ color: 'var(--neutral-700)' }}>
                        {selectedSlot.label} - {selectedSlot.price === 0 ? 'FREE' : `₹${selectedSlot.price}`}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="p-6">
                ...
                <div className="space-y-3">
                  <Button
                    ...
                    className="w-full"
                  >
                    {cartStep === 'cart' ? 'Continue to Delivery' :
                     cartStep === 'delivery' ? 'Review Order' : 'Proceed to Checkout'}
                  </Button>

                  {cartStep !== 'cart' && (
                    <Button
                      onClick={() => setCartStep(cartStep === 'delivery' ? 'cart' : 'delivery')}
                      variant="outline"
                      size="lg"
                      className="w-full"
                    >
                ...
                {subtotal < 500 && (
                  <div className="mt-6 p-4 rounded-lg" style={{ backgroundColor: 'var(--info-50)' }}>
                    <div className="text-sm font-medium mb-2" style={{ color: 'var(--info-700)' }}>
                ...
                {/* Security Badge */}
                <div className="mt-6 flex items-center justify-center gap-2 text-sm" style={{ color: 'var(--neutral-600)' }}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
