...
const orderStatuses = {
  ...
  shipped: { label: 'Out for Delivery', color: 'bg-indigo-100 text-indigo-800', icon: '🚚' },
    ...
    return (
      <div className="min-h-screen bg-secondary-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-secondary-600">Loading your orders...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-secondary-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-bold text-secondary-900 mb-2">My Orders</h1>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {['all', 'delivered', 'shipped', 'processing', 'pending'].map(status => (
            ...
            >
              {status === 'all' ? 'All Orders' : orderStatuses[status]?.label || status}
            </button>
          ))}
        </div>
        ...
        {filteredOrders.length > 0 ? (
          <div className="space-y-6">
            {filteredOrders.map((order) => (
              <div key={order.id} className="bg-white rounded-xl border border-secondary-200 overflow-hidden hover:shadow-lg transition-shadow duration-200">
                {/* Order Header */}
                <div className="p-6 border-b border-secondary-100">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`inline-flex items-center px-3 py-2 rounded-full text-sm font-medium ${orderStatuses[order.status].color}`}>
                ...
                <div className="p-6">
                  <div className="flex flex-col lg:flex-row justify-between gap-6">
                    <div className="flex-1">
                      <h4 className="font-semibold text-secondary-900 mb-2">Delivery Address</h4>
                      <div className="text-sm text-secondary-600">
                        <p className="font-medium text-secondary-800">{order.deliveryAddress.name}</p>
                        <p>{order.deliveryAddress.address}</p>
                        <p>{order.deliveryAddress.city}</p>
                      </div>
                      <div className="mt-3">
                        <span className="text-sm text-secondary-600">Payment: </span>
                        <span className="text-sm font-medium text-secondary-800">{order.paymentMethod}</span>
                      </div>
                    </div>
