    ...
    >
      ...
      {(change || trend) && (
        <div className='mt-4 flex items-center space-x-2'>
          {change && (
            <div
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${bgColor} ${changeColor}`}
            >
          ...
          {trend && <span className='text-xs text-secondary-500'>{trend}</span>}
        </div>
      )}
    </div>
  );
