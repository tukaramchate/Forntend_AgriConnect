...
const SmartSuggestions = ({ searchQuery, onSuggestionClick, recentSearches = [], userPreferences = {} }) => {
  ...
  const generateSmartSuggestions = (query) => {
    const baseSuggestions = [
      ...
      { type: 'category', text: 'Organic Vegetables', icon: '🥬', confidence: 0.9 },
      { type: 'category', text: 'Fresh Fruits', icon: '🍎', confidence: 0.85 },
      { type: 'category', text: 'Dairy Products', icon: '🥛', confidence: 0.8 },
      
      // Product suggestions
      { type: 'product', text: 'Tomatoes', icon: '🍅', confidence: 0.95, description: 'Fresh organic tomatoes' },
      { type: 'product', text: 'Apples', icon: '🍎', confidence: 0.9, description: 'Crisp seasonal apples' },
      { type: 'product', text: 'Milk', icon: '🥛', confidence: 0.88, description: 'Farm fresh milk' },
      
      // Farmer suggestions
      { type: 'farmer', text: 'Ramesh Kumar Farm', icon: '👨‍🌾', confidence: 0.75, description: 'Organic vegetable specialist' },
      { type: 'farmer', text: 'Green Valley Dairy', icon: '🏡', confidence: 0.7, description: 'Premium dairy products' },
      
      // Seasonal suggestions
      { type: 'seasonal', text: 'Winter Vegetables', icon: '❄️', confidence: 0.8, description: 'Fresh winter harvest' },
      { type: 'seasonal', text: 'Citrus Fruits', icon: '🍊', confidence: 0.85, description: 'Vitamin C rich fruits' },
    ];
  ...
  const visualSearchSuggestions = [
    ...
    { text: 'Voice search: "Find organic tomatoes"', icon: '🎤', action: 'voice' },
  ];
  ...
  return (
    <div className="p-6" style={{ backgroundColor: 'var(--neutral-50)' }}>
      <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant={suggestion.type === 'product' ? 'success' : suggestion.type === 'farmer' ? 'warning' : 'info'} className="text-xs">
                            {suggestion.type}
                          </Badge>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {recentSearches.slice(0, 6).map((search, index) => (
                ...
                >
                  <svg className="w-4 h-4" style={{ color: 'var(--neutral-500)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {trendingSuggestions.map((item, index) => (
                <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => onSuggestionClick(item.text, 'trending')}>
                  <CardBody className="text-center p-4">
                    <div className="text-3xl mb-2">{item.icon}</div>
                    <div className="font-medium mb-1" style={{ color: 'var(--neutral-900)' }}>
                      {item.text}
                    </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {visualSearchSuggestions.map((option, index) => (
                <Card key={index} className="cursor-pointer hover:shadow-md transition-all hover:scale-105" onClick={() => handleSpecialAction(option.action)}>
        ...
        {/* AI Recommendations */}
        {!searchQuery && userPreferences && Object.keys(userPreferences).length > 0 && (
          <div className="mt-8 p-6 rounded-lg" style={{ backgroundColor: 'var(--primary-50)' }}>
